...
export class SpotifySession extends DurableObject<Bindings> {
    ...
    private async handle(request: Request): Promise<Response> {
        try {
            ...
            return Response.json(
                {error: failure.code},
                {status: failure.status, headers: {"Retry-After": String(failure.retry)}},
            );
        }
    }
    ...
    private async network(url: string, init: RequestInit = {}): Promise<Response> {
        ...
        try {
            ...
            throw new UpstreamError("spotify_unavailable");
        } finally {
    ...
    private async exchange(body: URLSearchParams, previous?: Tokens): Promise<Tokens> {
        ...
        if (
            ...
            throw new UpstreamError("spotify_invalid_response");
        return {
    ...
    private async notify(): Promise<void> {
        ...
        try {
            ...
            await this.network(
                `https://maker.ifttt.com/trigger/${encodeURIComponent(this.env.IFTTT_EVENT)}/with/key/${encodeURIComponent(this.env.IFTTT_KEY)}`,
                {
                    method: "POST",
                    headers: {"Content-Type": "application/json"},
                    body: JSON.stringify({
                        value1: link.href,
                        value2: "Spotify needs reauthorization. This link expires in 24 hours.",
                    }),
                },
            );
        } catch {
            /* Retry only on a later request after the persisted cooldown. */
        }
    }
    ...
    private async current(): Promise<Response> {
        ...
        try {
            ...
            if (response.status !== 204 && (!playback || typeof playback.is_playing !== "boolean"))
                throw new UpstreamError("spotify_invalid_response");
            ...
            if (playback?.is_playing && match) {
                ...
                const cached = await this.ctx.storage.get<{value: Parameters<typeof transform>[1]; at: number}>(
                    "context",
                );
                ...
                if (cached && cachedKey === key && now - cached.at < 3600_000) context = cached.value;
                    ...
                    try {
                        const extra = await this.network(
                            `https://api.spotify.com/v1/${match[1]}s/${match[2]}${match[1] === "playlist" ? "?fields=name,type,external_urls" : ""}`,
                            {headers: {Authorization: `Bearer ${tokens.access}`}},
                        );
                        if (extra.status === 429) await this.check(extra);
                        context = extra.ok
                            ? ((await extra.json()) as NonNullable<Parameters<typeof transform>[1]>)
                            : null;
    ...
    private failureResponse(failure: Failure, snapshot?: Snapshot): Response {
        ...
        if (
            ["spotify_unavailable", "spotify_rate_limited"].includes(failure.error) &&
            snapshot &&
            Date.now() - snapshot.at <= 30_000
        ) {
    ...
    private async login(request: Request): Promise<Response> {
        ...
        if (!recovery || recovery.expiresAt <= Date.now() || ticket !== recovery.ticket)
            return new Response("This login link has expired. Open the widget to request a new notification.", {
                status: 400,
            });
        ...
        url.search = new URLSearchParams({
            ...
            response_type: "code",
            redirect_uri: new URL("/callback", this.env.PUBLIC_BASE_URL).href,
            state: login.state,
        ...
        return new Response(null, {
            ...
            headers: {
                Location: url.href,
                "Set-Cookie": `${COOKIE}=${login.browser}; Secure; HttpOnly; SameSite=Lax; Path=/; Max-Age=600`,
            },
        });
    }
    ...
    private async callback(request: Request): Promise<Response> {
        ...
        if (
            ...
            url.searchParams.get("state") !== login.state ||
            cookie !== login.browser
        )
            return new Response("Invalid or expired authorization. Reopen your IFTTT link.", {status: 400});
        await this.ctx.storage.delete("login");
        const code = url.searchParams.get("code");
        if (url.searchParams.has("error") || !code)
            return new Response("Authorization cancelled. Reopen your IFTTT link to try again.", {status: 400});
        ...
        const profile = await this.network("https://api.spotify.com/v1/me", {
            headers: {Authorization: `Bearer ${tokens.access}`},
        });
        await this.check(profile);
        const owner = (await profile.json()) as {id?: string};
        if (owner.id !== this.env.SPOTIFY_USER_ID)
            return new Response("Please authorize the Spotify account configured for this widget.", {status: 403});
        await this.ctx.storage.put("tokens", tokens);
        await this.ctx.storage.delete(["recovery", "failure", "playback", "context", "contextKey"]);
        return new Response("Spotify is connected. You can close this page.", {
            headers: {"Set-Cookie": `${COOKIE}=; Secure; HttpOnly; SameSite=Lax; Path=/; Max-Age=0`},
        });
    }
}
