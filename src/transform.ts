...
export interface Playback {
  ...
  item?: (Entity & { duration_ms?: number; explicit?: boolean; album?: Entity; show?: Entity; artists?: Entity[] }) | null;
}
...
export function transform(playback: Playback | null, context: Entity | null | undefined, missingAlbumUrl: string) {
  ...
  const artists = episode ? [item.show?.publisher || item.show?.name || "Podcast"] : (item.artists || []).map(a => a.name || "Unknown artist");
  ...
  return {
    playing: true,
    device: { name: playback.device?.name || "", type: deviceType.charAt(0).toUpperCase() + deviceType.slice(1).toLowerCase() },
    player: {
      ...
      progress: item.duration_ms && item.duration_ms > 0 ? Math.max(0, Math.min(1, (playback.progress_ms ?? 0) / item.duration_ms)) : 0
    },
    track: {
      context: contextEntity?.name ? { name: contextEntity.name, type: contextEntity.type || (episode ? "show" : "album"), url: spotifyUrl(contextEntity.external_urls?.spotify) } : null,
      artists: { names: artists, url: spotifyUrl(episode ? item.show?.external_urls?.spotify : item.artists?.[0]?.external_urls?.spotify) },
