...
const get = (path = "/current", headers = {}) => worker.fetch(new Request(`https://mgic.example${path}`, { headers }), bindings);
...
const track = { is_playing: true, progress_ms: 500, device: { type: "computer", name: "Laptop", volume_percent: 50 }, item: { type: "track", name: "Track", duration_ms: 1000, album: { name: "Album", type: "album", images: [{ url: "https://i.scdn.co/image/a" }] }, artists: [{ name: "Artist" }] } };
let fetchMock: ReturnType<typeof vi.spyOn>;
beforeEach(() => { fetchMock = vi.spyOn(globalThis, "fetch").mockRejectedValue(new Error("Unexpected network request")); });
...
it("keeps health, unknown routes, and methods independent of Spotify", async () => {
  ...
  expect((await worker.fetch(new Request("https://mgic.example/current", { method: "POST" }), bindings)).status).toBe(405);
...
it("returns the Worker's artwork URL when Spotify has no image", async () => {
  await seed();
  fetchMock.mockImplementation(() => Response.json({ ...track, item: { ...track.item, album: { ...track.item.album, images: [] } } }));
...
it("restricts CORS without requiring browser visibility headers", async () => {
  await seed(); fetchMock.mockImplementation(() => Response.json(track));
  ...
  expect((await get("/current", { Origin: "https://untrusted.example" })).headers.get("Access-Control-Allow-Origin")).toBeNull();
...
it("keeps the existing refresh token when refresh omits a replacement", async () => {
  await seed(true);
  fetchMock.mockImplementationOnce(() => Response.json({ access_token: "new", expires_in: 3600 })).mockImplementationOnce(() => new Response(null, { status: 204 }));
...
it("retries a playback 401 only once", async () => {
  await seed();
  fetchMock.mockImplementationOnce(() => new Response(null, { status: 401 })).mockImplementationOnce(() => Response.json({ access_token: "new", expires_in: 3600 })).mockImplementationOnce(() => new Response(null, { status: 401 }));
  expect((await get()).status).toBe(503);
  expect(fetchMock).toHaveBeenCalledTimes(3);
});

it.each([403, 429, 500])("backs off on Spotify %s without calling it on each poll", async (status) => {
  await seed(); fetchMock.mockImplementation(() => new Response(null, { status, headers: { "Retry-After": "120" } }));
...
it("completes browser-bound authorization and rejects callback replay", async () => {
  ...
  fetchMock.mockImplementationOnce(() => Response.json({ access_token: "access", refresh_token: "refresh", expires_in: 3600 })).mockImplementationOnce(() => Response.json({ id: "goofyshnoofy" }));
...
it("maps tracks, podcasts without publisher, private sessions and unknown items safely", () => {
  expect(transform(track, undefined, missingAlbumUrl)).toMatchObject({ playing: true, player: { progress: 0.5 }, track: { name: "Track" }, device: { type: "Computer" } });
  expect(transform({ is_playing: true, item: { type: "episode", name: "Episode", show: { name: "Show" } } }, undefined, missingAlbumUrl)).toMatchObject({ track: { artists: { names: ["Show"] }, context: { name: "Show" }, image: missingAlbumUrl } });
...
it.each(["wrong-owner", "cancel", "expired"])("rejects %s authorization without changing tokens", async (mode) => {
  ...
  fetchMock.mockImplementationOnce(() => Response.json({ access_token: "wrong", refresh_token: "wrong", expires_in: 3600 }))
    .mockImplementationOnce(() => Response.json({ id: "someone-else" }));
  const response = await get(`/callback?state=state&${mode === "cancel" ? "error=access_denied" : "code=code"}`, { Cookie: "__Host-mgic-oauth=browser" });
...
it("fails closed for missing configuration and unexpected hosts", async () => {
  expect((await worker.fetch(new Request("https://mgic.example/current"), { ...bindings, SPOTIFY_CLIENT_ID: "" })).status).toBe(503);
